#!/usr/bin/env node
// Claude Matrix Display – Bridge  (Mac / Windows / Linux, läuft auch als
...
//
// Als Hook nutzbar:  claude-matrix event <idle|working|waiting>
...
const DEFAULT_CONFIG = {
  mqtt: { url: "mqtt://localhost:1883", username: "", password: "", topic: "claude/matrix/state", configTopic: "claude/matrix/config", retain: true },
  http: { port: 8718 },
  pollSeconds: 30,
...
// ---- Hook-Modus:  claude-matrix event <status> ----------------------------
function runHook(status) {
  const url = process.env.CLAUDE_MATRIX_URL || `http://localhost:${cfg.http.port}/event`;
...
// ---- Autostart beim Login:  claude-matrix autostart <enable|disable> -------
// Plattformübergreifend; zeigt auf die laufende Binärdatei (im Dev auf node).
const AUTOSTART_LABEL = "com.nuvooo.claude-matrix";
...
<plist version="1.0"><dict>
  ...
  <key>KeepAlive</key><false/>
      ...
      const args = enable
        ? ["add", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "ClaudeMatrix", "/t", "REG_SZ", "/d", autostartTargetCmd(), "/f"]
        : ["delete", "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run", "/v", "ClaudeMatrix", "/f"];
      execFileSync("reg", args, { stdio: "ignore" });
    } else {
      const dir = path.join(homedir(), ".config", "autostart");
      const file = path.join(dir, "claude-matrix.desktop");
      if (enable) {
        mkdirSync(dir, { recursive: true });
...
function startServer() {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`[bridge] Hooks ->  ${url}event  (claude-matrix event <status>)`);
